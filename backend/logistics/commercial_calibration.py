...
def _error_values(items: Iterable[CalibrationObservation]) -> list[Decimal]:
    # Positive error means the model over-predicted realized margin.
    return [item.predicted_margin - item.actual_margin for item in items if item.actual_margin is not None and item.predicted_margin is not None]
...
def calibrate_observations(observations: Iterable[CalibrationObservation], *, min_sample: int = 10, drift_win_rate_delta: float = 0.15, drift_error_delta: Decimal = Decimal("100.00")) -> CalibrationReport:
    ...
    terminal = [item for item in items if item.outcome in {"won", "lost"}]
    errors = _error_values(items)
    bands = tuple(_band([item for item in items if priority_band(item.priority_score) == name], name, min_sample) for name in BANDS)
    ...
    for left, right in zip(sufficiently_sampled, sufficiently_sampled[1:]):
        ...
        if left.mean_abs_prediction_error is not None and right.mean_abs_prediction_error is not None and abs(left.mean_abs_prediction_error - right.mean_abs_prediction_error) >= drift_error_delta:
...
def load_calibration_observations(conn: object, *, tenant_id: object, limit: int = 5000) -> list[CalibrationObservation]:
    ...
    rows = conn.execute(
        ...
        WITH latest AS (
          SELECT DISTINCT ON (h.opportunity_id)
                 o.priority_score, o.estimated_margin, h.actual_margin, h.outcome
        ...
        SELECT priority_score, estimated_margin, actual_margin, outcome
          FROM latest
    ...
    return [CalibrationObservation(priority_score=float(row[0]) if row[0] is not None else 0.0, predicted_margin=Decimal(str(row[1])) if row[1] is not None else None, actual_margin=Decimal(str(row[2])) if row[2] is not None else None, outcome=row[3]) for row in rows]
