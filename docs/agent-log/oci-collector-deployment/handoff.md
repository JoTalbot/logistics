...
## Cloud LLMBalancer migration

Inspected active AIOS service and provider code; existing /api/v1/aios/ask had local fallback and prompt[:4000]/small output limits. Added opt-in cloud_only/json_mode without changing default-client behavior; excluded local/RPA/emergency classes, no prompt cache, structured4096-token budget. Server files backed up under /root/logistics-balancer-backup; portable diff in deploy/llmbalancer-cloud-only.patch; AIOS restarted to load changes. Logistics client checks capabilities, caps full goal3800chars, records actual provider, preserves ID/evidence checks and retries. Initial JSON errors caused18 retries; subsequent batches complete.62 unit tests/4 skipped; additional AIOS no-local/cache/options tests passed. 752 pending jobs moved to route fingerprint, old completed left unchanged. Ollama Qwen unloaded, embedding service untouched; latest available RAM4.2GiB. API keys remain solely in original AIOS runtime secrets; texts now go to cloud providers per explicit user request. Documentation deploy/LLMBALANCER.md.
