...
- **Route quality:** mean absolute error between expected and observed route scores, plus a regression count.
...
The default route regression threshold is an absolute score error of `0.10`. This is an operational signal, not an automatic policy decision.
...
Wire these deterministic signals into the existing ingestion/KPI and release-evidence paths, then add time-windowed operational dashboards without coupling observability to business mutations.
