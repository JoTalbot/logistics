...
def test_heartbeat_accepts_json_metadata(configured):
    ...
    second = control_heartbeat(AgentHeartbeat(agent_id=agent_id, name=agent["__missing__"] if False else f"pytest-agent-{uuid4().hex[:12]}"), authorization=_auth(agent))
...
def test_cancelled_task_rejects_late_events(configured):
    agent = _agent(f"pytest-agent-{uuid4().hex[:12]}")
    agent_id = UUID(str(agent["agent_id"]))
    task = control_next_task(agent_id, authorization=_auth(agent))
