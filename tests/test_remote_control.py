...
def _agent(name: str, tenant_id=None) -> dict[str, object]:
    return control_heartbeat(
        AgentHeartbeat(name=name, tenant_id=tenant_id, metadata={"hostname": name, "source": "pytest"}),
...
def test_heartbeat_accepts_json_metadata(configured):
    ...
    second = control_heartbeat(
        AgentHeartbeat(agent_id=agent_id, name=name, metadata={"hostname": name, "source": "pytest-2"}),
...
def test_safe_task_lifecycle(configured):
    ...
    created = control_create_task(
        TaskRequest(agent_id=agent_id, command="uname -a", idempotency_key=f"k-{uuid4().hex}"),
    ...
    assert control_complete(
        UUID(str(task["id"])),
        CompleteRequest(returncode=0, stdout="Linux arm-server-01"),
        authorization=_auth(agent),
    )["status"] == "succeeded"
...
def test_task_tenant_must_match_agent_tenant(configured):
    ...
    with pytest.raises(HTTPException) as excinfo:
        control_create_task(
            TaskRequest(agent_id=UUID(str(agent["agent_id"])), tenant_id=_tenant(), command="pwd"),
            authorization=f"Bearer {OPERATOR_TOKEN}",
        )
    assert excinfo.value.status_code == 403
...
def test_credential_is_stored_as_hash_not_plaintext(configured):
    ...
    with psycopg.connect(os.environ["DATABASE_URL"]) as conn:
        row = conn.execute(
            "SELECT credential_hash,credential_created_at FROM remote_agents WHERE id=%s",
